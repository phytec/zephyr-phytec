...
static const struct spi_config spi_cfg = {
	...
	.cs = {.gpio = SPI_CS_GPIOS_DT_SPEC_GET(ST25R3911B_NODE), .delay = T_NCS_SCLK}};
...
static bool cmd_is_valid(uint8_t cmd)
{
	return ((!((cmd >= ST25R3911B_CMD_SET_DEFAULT) && (cmd <= ST25R3911B_CMD_ANALOG_PRESET))) ||
		(!((cmd >= ST25R3911B_CMD_MASK_RECEIVE_DATA) &&
		   (cmd <= ST25R3911B_CMD_TEST_ACCESS))));
}

int st25r3911b_spi_init(void)
{
	LOG_DBG("Initializing. SPI device: %s, CS GPIO: %s pin %d", spi_dev->name,
		spi_cfg.cs.gpio.port->name, spi_cfg.cs.gpio.pin);
...
int st25r3911b_multiple_reg_write(uint8_t start_reg, uint8_t *val, size_t len)
{
	...
	if ((!val) || (!reg_is_valid(start_reg)) || (!reg_is_valid(start_reg + (len - 1)))) {
		return -EINVAL;
	}
	...
	const struct spi_buf tx_bufs[] = {{.buf = &cmd, .len = 1}, {.buf = val, .len = len}};
...
int st25r3911b_multiple_reg_read(uint8_t start_reg, uint8_t *val, size_t len)
{
	...
	const struct spi_buf tx_buf = {.buf = &start_reg, .len = 1};
	const struct spi_buf_set tx = {.buffers = &tx_buf, .count = 1};

	/* Read register value. */
	const struct spi_buf rx_bufs[] = {{.buf = NULL, .len = 1}, {.buf = val, .len = len}};
...
int st25r3911b_fifo_write(uint8_t *data, size_t length)
{
	...
	const struct spi_buf tx_bufs[] = {{.buf = &cmd, .len = 1}, {.buf = data, .len = length}};
	const struct spi_buf_set tx = {.buffers = tx_bufs, .count = ARRAY_SIZE(tx_bufs)};
...
int st25r3911b_fifo_read(uint8_t *data, size_t length)
{
	...
	/* Read FIFO. */
	const struct spi_buf rx_bufs[] = {{.buf = NULL, .len = 1}, {.buf = data, .len = length}};
	const struct spi_buf_set rx = {.buffers = rx_bufs, .count = ARRAY_SIZE(rx_bufs)};
