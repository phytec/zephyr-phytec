...
#if ICM40627_BUS_I2C
static int icm40627_bus_check_i2c(const union icm40627_bus *bus)
{
	return i2c_is_ready_dt(&bus->i2c) ? 0 : -ENODEV;
}

static int i2c_read_bank(const union icm40627_bus *bus, uint8_t reg, uint8_t bank, uint8_t *buf,
			 size_t len)
{
...
static int icm40627_reg_read_i2c(const union icm40627_bus *bus, uint16_t reg, uint8_t *data,
				 size_t len)
{
	int res = 0;
...
static int i2c_write_bank(const union icm40627_bus *bus, uint16_t reg, uint8_t bank, uint8_t buf)
{
	int res = i2c_reg_write_byte_dt(&bus->i2c, REG_REG_BANK_SEL, bank);
...
static int icm40627_reg_write_i2c(const union icm40627_bus *bus, uint16_t reg, uint8_t data)
{
	int res = 0;
	uint8_t bank = FIELD_GET(REG_BANK_MASK, reg);
	uint8_t address = FIELD_GET(REG_ADDRESS_MASK, reg);
...
static int icm40627_reg_update_i2c(const union icm40627_bus *bus, uint16_t reg, uint8_t mask,
				   uint8_t val)
{
