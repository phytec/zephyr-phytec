...
- **Press the user button** — the on-board RGB LED advances to the next color in
  the palette, and that color is broadcast over BLE so every other board mirrors
  it.
...
The RGB LED is driven through the `led0`/`led1`/`led2` GPIO aliases (active-low,
handled by the devicetree flags). The palette cycles through:
...
> **Note:** On some boards the HDC1010 has been observed to be faulty. If
> temperature/humidity reads `---`, that sensor — not the firmware — is the
> likely cause.
...
### E-paper SPI → SPIM overlay (`boards/reel_board.overlay`)
...
```shell
west build -b reel_board zephyr-phytec/samples/boards/phytec/reel_board/ble_broadcast
west flash
```

For revision 2 hardware, use the revision suffix:

```shell
west build -b reel_board@2 zephyr-phytec/samples/boards/phytec/reel_board/ble_broadcast
```

Repeat for each board you want to synchronize.
...
- A SEGGER J-Link (on-board) or compatible programmer for flashing.
